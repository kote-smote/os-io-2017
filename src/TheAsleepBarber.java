...
public class TheAsleepBarber {
    ...
    static Semaphore customerArrived;
    static Semaphore customerEnter;
    static boolean barberIsSleeping;
    static int customersNum;
    ...
    static class Barber extends TemplateThread {
        ...
        public void execute() throws InterruptedException {
            ...
            customerArrived.acquire(); // ceka da pristigne klient
            state.barberCallCustomer();
            enter.release(); // mu dava znak na klientot da vleze
            customerEnter.acquire(); // ceka duri ne vleze klient za da go potstrizi
            state.cutHair();
            done.release();
            synchronized (lock) {
    ...
    static class Consumer extends TemplateThread {
        ...
        public void execute() throws InterruptedException {
            ...
            customerArrived.release(); // dava znak deka pristignal
            synchronized (lock) {
                // dokolku e pettiot, go budi berberot
                if (++customersNum == 5)
                    wakeUp.release();
            }
            enter.acquire(); // bara dozvola da vleze
            state.customerEntry(); // koga ke bide povikan, vleguva
            customerEnter.release(); // vlegol vo berbernicata i e spremen za potstrizuvanje
            done.acquire(); // ceka da zavrsi potstrizuvanjeto
            state.customerPay();  // koga ke go potstrizat, plakja
        }
    }
